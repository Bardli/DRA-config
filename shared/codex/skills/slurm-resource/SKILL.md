---
name: slurm-resource
description: Display the Slurm accounts, GPU profiles, and partitions the user can request on the current Alliance Canada cluster. A quick reference card. Use when the user asks what GPUs or accounts are available.
allowed-tools: Bash(whoami), Bash(hostname *), Bash(sshare *), Bash(sacctmgr *), Bash(sinfo *)
---
...
`rpp-<pi>` (RAC-awarded, higher priority). The `ccdb-clusters` skill's `pick-gpu-account.sh` ranks
them by FairShare and prefers RRG/RPP.
...
The `%G` (gres) column shows GPU types — e.g. `gpu:h100:4`,
...
- **GPU resources** — a table of GPU type / VRAM / per-node count / max time. On Fir, request the
  GPU only with `--gpus-per-node=<gpu_type>:<count>` — full `h100` (80 GB) or a MIG slice
  `nvidia_h100_80gb_hbm3_{1g.10gb,2g.20gb,3g.40gb}`. Never use `--partition`, `--gres`, or
  `--constraint` to choose the GPU.
- **Tips** — default to the smallest MIG slice that fits; use `slurm-status` for live availability;
  match CPUs to the GPU break-even (see `ccdb-clusters` references for the per-cluster page).
